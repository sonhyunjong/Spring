 ...
 *  -인터셉터를 구형하기 위해서는 HandlerInterceptorAdapter 클래스를 상속하는 방법으로 구현해야한다.
 *  
 *  필터와의 차이점
 *   -필터는 servlet 수행 전에 실행된다.(wed.xml에 설정)
 *   -인터셉터 DispatcherServlet 수행 후 컨트롤러에 요청을 넘기기 전에 실행된다(servlet-context.xml에 설정)
 */
...
public class LoginCheckInterceptor extends HandlerInterceptorAdapter {
	...
	public void afterConcurrentHandlingStarted(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		//비동기 요청시 실행되는 postHandle과 afterCompletion이 수행되지 않고 afterConcurrentHandleingStarted 메소드가 실행된다.
